...
def server(input, output, session):
    ...
    # Reaktívne hodnoty pre filtrované dáta
    filtered_data = reactive.Value(df)
    diagnozy = sorted(df["diagnoza MKCH-10"].dropna().unique())
    diagnozy.insert(0, "Všetky")  # Pridať možnosť "Všetky" pre žiadny filter
    ...
    def generuj_grafy(df, pohlavie=None, diagnoza=None, vek_od=None, vek_do=None):
        ...
        # Vždy pridáme stĺpec diagnoza_ano_nie, aby bol dostupný v dátach
        ...
        for mutacia in mutacie:
            ...
            # 1. Distribúcia genotypov – bez fill podľa diagnózy, len čistý count
            ...
            # 3. Genotypy podľa pohlavia (barplot)
            ...
            # 4. Genotypy podľa diagnózy (barplot) – len ak je diagnóza zvolená
            if fill_col:
            ...
            print(f"[DEBUG - HTML sekcia]: {sekcia_html}")

        final_html = '<div class="graf-kontajner" >' + ''.join(grafy_html) + '</div>'
        print(f"[DEBUG - Celkový HTML]: {final_html}")  # Debug výpis celého HTML
    ...
    def vykonaj_chi_kvadrat_testy(df, diagnoza=None, vek_od=None, vek_do=None):
        ...
        for mutacia in mutacie:
            ...
            # Pridávanie podmienok do popisu
            if vek_od is not None:
            ...
            # Chí-kvadrát: mutácia × diagnóza (špecifická alebo pečeňová)
            if diagnoza and diagnoza.strip() != "" and diagnoza != "Všetky":
    ...
    def page_ui():
        if input.page() == "Úvod":
            return ui.TagList(
                ui.p("Vitajte v aplikácií, ktorá pomáha lekárom a výskumníkom analyzovať genetické dáta pacientov a ich diagnózy."),
                ui.p("Kľúčové funkcie:"),
                ui.p("   - Obsahuje očistený dataset : Opravené chyby a nepresnosti v údajoch (napr. chýbajúce hodnoty)."),
                ui.p("   - Analyzuje gény: Kontroluje, či rozloženie HFE génov je v norme (Hardy-Weinbergova rovnováha)."),
                ui.p("   - Určuje predispozície: Zisťuje, koľko pacientov má gény pre hemochromatózu (dedičnú chorobu)."),
    ...
    def allele_info_table():
        return generate_hw_table(df, lambda r, c: {
            ...
            "Celkový počet alel": r.get("allele_info", {}).get("total_alleles") if r and r.get(
                "allele_info") is not None else "N/A",
            "Počet normálnych alel": r.get("allele_info", {}).get("normal_alleles") if r and r.get(
                "allele_info") is not None else "N/A",
            "Počet mutovaných alel": r.get("allele_info", {}).get("mutant_alleles") if r and r.get(
                "allele_info") is not None else "N/A",
            "Frekvencia normálnej alely (p)": f"{r.get('allele_info', {}).get('allele_p', 0):.4f}" if r and r.get(
                "allele_info") is not None else "N/A",
            "Frekvencia mutovanej alely (q)": f"{r.get('allele_info', {}).get('allele_q', 0):.4f}" if r and r.get(
                "allele_info") is not None else "N/A",
            "Dôvod N/A": r.get("reason_na",
                               "Dostatočné dáta") if r is not None else "Chyba pri výpočte (pravdepodobne nedostatok dát)"
        })
    ...
    def expected_values_table():
        return generate_hw_table(df, lambda r, c: {
            "Mutácia": r.get("Mutácia", c),
            "Normal": round(r.get("expected", {}).get("normal"), 2) if r and r.get("expected") is not None and r.get(
                "expected").get("normal") is not None else "N/A",
            "Heterozygot": round(r.get("expected", {}).get("heterozygot"), 2) if r and r.get(
                "expected") is not None and r.get("expected").get("heterozygot") is not None else "N/A",
            "Mutant": round(r.get("expected", {}).get("mutant"), 2) if r and r.get("expected") is not None and r.get(
                "expected").get("mutant") is not None else "N/A",
            "Dôvod N/A": r.get("reason_na",
                               "Dostatočné dáta") if r is not None else "Chyba pri výpočte (pravdepodobne nedostatok dát)"
        })
    ...
    def chi2_test_table():
        return generate_hw_table(df, lambda r, c: {
            "Mutácia": r.get("Mutácia", c),
            "Chi-kvadrát test": round(r.get("chi2_results", {}).get("chi2"), 4) if r and r.get(
                "chi2_results") is not None and r.get("chi2_results").get("chi2") is not None else "N/A",
            "Stupne voľnosti": r.get("chi2_results", {}).get("df") if r and r.get(
                "chi2_results") is not None else "N/A",
            "p-hodnota": round(r.get("chi2_results", {}).get("p_value"), 4) if r and r.get(
                "chi2_results") is not None and r.get("chi2_results").get("p_value") is not None else "N/A",
            "Dôvod N/A": r.get("reason_na",
                               "Dostatočné dáta") if r is not None else "Chyba pri výpočte (pravdepodobne nedostatok dát)"
        })
    ...
    def hw_results_table():
        return generate_hw_table(df, lambda r, c: {
            ...
            "Výsledok": (
                ...
                if r and r.get("chi2_results") is not None and r.get("chi2_results").get("p_value") is not None and
                   r["chi2_results"]["p_value"] > 0.05
                else (
    ...
    def predisposition_summary_table():
        ...
        predisposition_df.loc[predisposition_df[
                                  "Skupina"] == "Celkový počet pacientov", "Percento (%)"] = ""  # Or you could use None, or a dash "-"
